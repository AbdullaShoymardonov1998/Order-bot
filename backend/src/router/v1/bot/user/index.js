...
router.get("/get-by-telegram-id", joi.query(validator.get), async (req, res, next) => {
    try {
        return res.json({
            status: STATUS_SUCCESS,
...
router.patch("/language", joi.body(validator.changeLanguage), async (req, res, next) => {
    try {
        return res.json({
            status: STATUS_SUCCESS,
...
router.delete("/cart", joi.query(validator.emptyCart), async (req, res, next) => {
    try {
        return res.json({
            status: STATUS_SUCCESS,
...
router.post("/location", joi.body(validator.location), async (req, res, next) => {
    try {
        return res.json({
            status: STATUS_SUCCESS,
